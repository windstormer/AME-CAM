...
class Design_CAM(object):
    def __init__(self, args, exp_name, encoder_model_type, gpuid):
        ...
        score_model = Res18_Scoring().cuda()
    ...
    def step(self, img):
        ...
        final_map, foreground, background = self.score_model(img, map_collect)
        ...
        return norm_map, final_map.detach().cpu(), confidence[0], MECAM_map.detach().cpu()
    ...
    def run_selected_case(self, loader):
        ...
        with torch.no_grad():
            for img_name, case_batch, seg_batch in test_bar:
                ...
                map_collect, final_map, confidence, MECAM_map = self.step(case_batch)
                input_image = case_batch[0].permute(1, 2, 0)
                norm_map, final_map, final_seg, MECAM_map = self.CAM_algo(input_image, map_collect, final_map, MECAM_map, img_name, confidence)
                ...
                log_file.writelines(f"Dice Score: {dice}\n")
                log_file.writelines(f"IOU Score: {iou}\n")
                log_file.close()
    ...
    def CAM_algo(self, input_image, map_collect, final_map, MECAM_map, img_name, confidence, output_hist=False):
        ...
        final_map  = (1-final_map) * confidence.numpy()
