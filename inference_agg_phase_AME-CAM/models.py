...
class Res18_Scoring(nn.Module):
    def __init__(self, pretrain_path=None):
        super(Res18_Scoring, self).__init__()
    ...
    def forward(self, input, map_collect):
        ...
        final_map = torch.sum(norm_mask*map_weight, dim=1, keepdim=True)
        norm_final_map = self.normalize(final_map)
        foreground = input_gray * norm_final_map
        foreground = torch.flatten(foreground, start_dim=1)
        background = input_gray * (1-norm_final_map)
        background = torch.flatten(background, start_dim=1)

        return final_map, foreground, background
