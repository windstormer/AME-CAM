...
class CNet(object):
    ...
    def step(self, data_batch, label_batch, no_cl=False):
        logits_collect, map_collect, final_map = self.model(data_batch.cuda())
        loss = 0
        ic_weight = [0.25, 0.5, 0.75, 1.0]
        loss_collect = []
        for idx, logits in enumerate(logits_collect):
            loss_collect.append(self.loss(logits, label_batch.cuda()).detach().cpu())
            loss += ic_weight[idx] * self.loss(logits, label_batch.cuda())
